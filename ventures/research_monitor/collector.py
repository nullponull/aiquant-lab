...
使い方:
    uv run python ventures/research_monitor/collector.py [--dry-run] [--no-classify] [--no-x]
...
DATA_DIR = PROJECT_ROOT / "ventures" / "research_monitor" / "data" / "research_inbox"
...
def fetch_x_via_subprocess(keywords: list[str], limit: int = 15) -> list[dict]:
    ...
    bridge = """
import json, sys, os
sys.path.insert(0, os.path.join(os.environ.get('PROJECT_ROOT', '/home/sol/aiquant-lab'), 'ventures', 'research_monitor'))
