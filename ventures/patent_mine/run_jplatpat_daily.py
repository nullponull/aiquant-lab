...
実行:
    # 今日のキーワードで自動実行
    uv run python ventures/patent_mine/run_jplatpat_daily.py
    # キーワード指定
    uv run python ventures/patent_mine/run_jplatpat_daily.py --keyword "ペット 自動給水 サイフォン"
    # カテゴリの全キーワードを順次（時間かかるので手動推奨）
    uv run python ventures/patent_mine/run_jplatpat_daily.py --category pet
"""
