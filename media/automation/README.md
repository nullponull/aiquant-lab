...
uv run python media/automation/publish_episode.py --dry-run --episode 0

# 特定エピソードを今すぐ強制投稿
uv run python media/automation/publish_episode.py --episode 0

# X だけ投稿しない (note のみ)
uv run python media/automation/publish_episode.py --episode 0 --no-x

# note だけ投稿しない (X のみ)
uv run python media/automation/publish_episode.py --episode 0 --no-note
```
...
```bash
uv run python media/automation/publish_episode.py 2>&1 | tee logs/publish-$(date +%Y%m%d_%H%M).log
```
